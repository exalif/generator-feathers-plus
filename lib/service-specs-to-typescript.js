      ...
      case 'array':
        ...
        if (primitiveArrayTypes.includes(itemsType)) {
          if (isValidEnumSchema(property.items)) {
            const { enumName, enumPath } = extractEnumsTypes(
              name,
              property.items.enum,
              ...
              property.localExistingEnumPath || null
            );

            !!enumPath
              ? existingTypescriptEnumsImports.add({ enumName, enumPath })
...
function extractEnumsTypes(name, propertyEnums, typescriptGlobalEnumsArr, typescriptEnums, propertyPath, reuseIdenticalEnums, numberEnumKeys = [], useExistingEnum = null, localExistingEnumPath = null) {
  if (isExistingEnumProvided(useExistingEnum, localExistingEnumPath)) {
    return { enumName: useExistingEnum, enumPath: localExistingEnumPath };
  }
...
function isExistingEnumProvided(useExistingEnum, localExistingEnumPath) {
  return !!useExistingEnum
    && typeof useExistingEnum === 'string'
    && !!localExistingEnumPath
    && typeof localExistingEnumPath === 'string';
}
