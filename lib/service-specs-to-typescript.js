...
module.exports = function serviceSpecsToTypescript(specsService, feathersSpec, feathersExtension, depth = 1, propertyPath = [], typescriptGlobalEnumsArr, typescriptEnums = [], typescriptEnumsImports = new Set(), generateTypeScriptEnums, reuseIdenticalEnums) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    switch (property.type) {
      ...
      case 'object':
        ...
        subTypes = serviceSpecsToTypescript(specsService, property, feathersExtension, ++depth, propertyPath, typescriptGlobalEnumsArr, typescriptEnums, typescriptEnumsImports, generateTypeScriptEnums, reuseIdenticalEnums);
...
function extractEnumsTypes(name, propertyEnums, typescriptGlobalEnumsArr, typescriptEnums, propertyPath, reuseIdenticalEnums) {
  ...
  console.log('reuse value', reuseIdenticalEnums)
  console.log('identical by values global', identicalEnumsInGlobalArrayByValues)

  if (reuseIdenticalEnums && identicalEnumsInGlobalArrayByValues.length > 0) {
    console.log('reusing global enum name');
  ...
  if (identicalEnumsInGlobalArray.length === 0) {
    console.log('found no identical in global array. Searching local array.');

    let identicalNewEnumsInArrayByValues = typescriptEnums.filter((enumItem => isEqual(enumItem.values, enumValues)));

    console.log('identical by values local', identicalNewEnumsInArrayByValues)

    if (reuseIdenticalEnums && identicalNewEnumsInArrayByValues.length > 0) {
      console.log('reusing local enum name');
