    ...
    switch (property.type) {
      case 'array':
        ...
        if (primitiveArrayTypes.includes(itemsType)) {
          if (!!property.items.enum && !!property.items.enum.length && (
...
function handleExistingEnumInArray(identicalArray, enumName, enumValues, propertyPath, typescriptEnums, numberEnumKeys = []) {
  identicalArray.forEach(enumItem => {
    if (
      (!numberEnumKeys.length && !isEqual(enumItem.values, enumValues)) ||
      (!!numberEnumKeys.length && !isEqual(enumItem.values, enumValues) && !isEqual(enumItem.keys, numberEnumKeys))
    ) {
