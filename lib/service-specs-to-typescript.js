...
module.exports = function serviceSpecsToTypescript (specsService, feathersSpec, feathersExtension, depth = 1) {
      ...
      default:
        if (property.type !== 'ID') {
          if (property.type === 'string' && !!property.enum && property.enum.length > 0) {
            const enumName = `${capitalize(name)}${enumSuffix}`;

            typescriptEnums.push({
              name: `${enumName}${enumSuffix}`,
              values: [...property.enum.map(value => camelCase(value))]
            });
  ...
  return { typescriptTypes, typescriptExtends, typescriptEnums };
};
