      ...
      case 'array':
        ...
        if (primitiveArrayTypes.includes(itemsType)) {
          if (!!property.items.enum && !!property.items.enum.length && property.items.enum.length > 0) {
            const enumName = extractEnumsTypes(name, property.items.enum, typescriptGlobalEnumsArr, typescriptEnums, propertyPath, reuseIdenticalEnums, property.items.type === 'number', property.items.enumKeys || []);
