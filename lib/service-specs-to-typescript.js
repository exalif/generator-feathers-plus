      ...
      case 'array':
        ...
        if (primitiveArrayTypes.includes(itemsType)) {
          if (!!property.items.enum && property.items.enum.length > 0) {
            const enumName = extractEnumsTypes(name, property.items.enum, typescriptGlobalEnumsArr, typescriptEnums, propertyPath);
      ...
      default:
        if (property.type !== 'ID') {
          if (property.type === 'string' && !!property.enum && property.enum.length > 0) {
            const enumName = extractEnumsTypes(name, property.enum, typescriptGlobalEnumsArr, typescriptEnums, propertyPath);
...
function extractEnumsTypes(name, propertyEnums, typescriptGlobalEnumsArr, typescriptEnums, propertyPath) {
