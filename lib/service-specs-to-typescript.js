...
module.exports = function serviceSpecsToTypescript(specsService, feathersSpec, feathersExtension, generateTypeScriptEnums, reuseIdenticalEnums, typescriptGlobalEnumsArr, depth = 1, propertyPath = [], typescriptEnums = [], typescriptEnumsImports = new Set(), existingTypescriptEnumsImports = new Set()) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    switch (property.type) {
      case 'array':
        ...
        if (primitiveArrayTypes.includes(itemsType)) {
          if (isValidEnumSchema(property.items)) {
            const { enumName, enumPath, relativePath } = extractEnumsTypes(
              name,
              property.items.enum,
              typescriptGlobalEnumsArr,
              typescriptEnums,
              propertyPath,
              reuseIdenticalEnums,
              property.items.enumKeys || [],
              property.items.useExistingEnum || null,
              property.items.existingEnumPath || null,
              property.items.existingEnumPathRelativeToModels === undefined ? true : property.items.existingEnumPathRelativeToModels
            );

            !!enumPath
              ? existingTypescriptEnumsImports.add({ enumName, enumPath, relativePath })
      ...
      case 'object':
        ...
        subTypes = serviceSpecsToTypescript(specsService, property, feathersExtension, generateTypeScriptEnums, reuseIdenticalEnums, typescriptGlobalEnumsArr, ++depth, propertyPath, typescriptEnums, typescriptEnumsImports, existingTypescriptEnumsImports);
        ...
        if (property.type === 'ID') {
          ...
          if (isValidEnumSchema(property)) {
            const { enumName, enumPath, relativePath } = extractEnumsTypes(
              ...
              reuseIdenticalEnums,
              property.enumKeys || [],
              property.useExistingEnum || null,
              property.existingEnumPath || null,
              property.existingEnumPathRelativeToModels === undefined ? true : property.existingEnumPathRelativeToModels
            );
  ...
  return {
    ...
    typescriptExtends,
    typescriptEnums,
    typescriptEnumsImports,
    existingTypescriptEnumsImports
  };
};

function extractEnumsTypes(name, propertyEnums, typescriptGlobalEnumsArr, typescriptEnums, propertyPath, reuseIdenticalEnums, numberEnumKeys = [], useExistingEnum = null, existingEnumPath = null, existingEnumPathRelativeToModels = true) {
  if (isExistingEnumProvided(useExistingEnum, existingEnumPath)) {
    return {
      enumName: useExistingEnum,
      enumPath: existingEnumPath,
      relativePath: existingEnumPathRelativeToModels
    };
  }
...
function isValidEnumSchema(propertyItem) {
  return !!propertyItem.enum
    ...
    && (
      isValidNumberEnum(propertyItem)
      || isValidStringEnum(propertyItem)
      || isExistingEnumProvided(propertyItem.useExistingEnum, propertyItem.existingEnumPath)
    );
}
...
function isExistingEnumProvided(useExistingEnum, existingEnumPath) {
  return !!useExistingEnum
    && typeof useExistingEnum === 'string'
    && !!existingEnumPath
    && typeof existingEnumPath === 'string';
}
