...
module.exports = function serviceSpecsToTypescript (specsService, feathersSpec, feathersExtension, depth = 1, propertyPath = [], typescriptGlobalEnumsArr, typescriptEnums = [], typescriptEnumsImports = []) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    switch (property.type) {
      ...
      case 'object':
        ...
        subTypes = serviceSpecsToTypescript(specsService, property, feathersExtension, ++depth, propertyPath, typescriptGlobalEnumsArr, typescriptEnums, typescriptEnumsImports);
...
function extractEnumsTypes(name, property, typescriptGlobalEnumsArr, typescriptEnums, propertyPath) {
  ...
  if (identicalEnumsInGlobalArray.length === 0) {
    ...
    if (identicalNewEnumsInArray.length === 0) {
      ...
      let identicalArray = identicalNewEnumsInArray;

      console.log('found identical in local array!', enumName, enumValues, identicalArray);
      ...
      identicalArray.forEach(enumItem => {
        if (!isEqual(enumItem.values, enumValues)) {
          ...
          console.log('value non identical! Traversing propertyPath', propertyPath);
    ...
    let identicalArray = identicalEnumsInGlobalArray;
    identicalArray.forEach(enumItem => {
      if (!isEqual(enumItem.values, enumValues)) {
        let tryName = enumName;
        let breakLoop = false;
