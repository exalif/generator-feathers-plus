...
module.exports = function serviceSpecsToTypescript (specsService, feathersSpec, feathersExtension, depth = 1, propertyPath = []) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    switch (property.type) {
      ...
      case 'object':
        ...
        subTypesStr = subTypes.typescriptTypes.map(str => `  ${str}`).join(`;${EOL}`).prepend();
        typescriptTypes.push(`${name}: {${EOL}${subTypesStr}${EOL}}`);
        break;
      default:
        if (property.type !== 'ID') {
          if (property.type === 'string' && !!property.enum && property.enum.length > 0) {
            let enumName = `${capitalize(property.title)}${capitalize(name)}${enumSuffix}`; // PropertyAttributeEnum
            let enumValues = property.enum;
            let identicalEnumsInArray = typescriptEnums.filter((enumItem => enumItem.name === enumName));

            if (identicalEnumsInArray.length === 0) {
              typescriptEnums.push({
                name: enumName,
                values: [...enumValues.map(value => camelCase(value))]
              });
            } else {
              identicalEnumsInArray.forEach(enumItem => {
            ...
            // Think about put back import of Enum if a previously existing one was reused
